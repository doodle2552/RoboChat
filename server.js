...
const authlFile = fs.readFileSync(path.join(__dirname, 'static', 'auth.js'));
...
const server = http.createServer((req,res) => {
    ...
    switch(req.url) {
      case '/register': return res.end(regHtmlFile);
        ...
        case '/auth.js': return res.end(authlFile);
...
function registerUser(req,res) {
  ...
  req.on('end', async function() {
    try {
      ...
      return res.end('Registration is succcesfull');
    }
    catch(e) {
      res.writeHead(500);
...
io.use((soket, next) => {
  const cookie = soket.handshake.auth.cookie;
  const credentionals = getCredentionals(cookie);
  if(!credentionals) {
    next(new Error(" no auth"));
  }
  soket.credentionals = credentionals;
  next();
})
